...
public class GridViewHelper
{
    ...
    private readonly bool useFooter;
    private readonly SortDirection groupSortDir;

    #endregion Fields

    public GridViewGroupList Groups => mGroups;

    public GridViewSummaryList GeneralSummaries => mGeneralSummaries;
    ...
    private bool EvaluateEquals(GridViewGroup g, object dataitem)
    {
        ...
        {
            if (g.ActualValues[i] == null && DataBinder.Eval(dataitem, g.Columns[i]) != null)
            {
                return false;
            }
            ...
            if (!g.ActualValues[i].Equals(DataBinder.Eval(dataitem, g.Columns[i])))
            {
                return false;
            }
        }
    ...
    private void GenerateGroupSummary(GridViewGroup g, GridViewRow row)
    {
        ...
        foreach (GridViewSummary s in g.Summaries)
        {
            if (s.Automatic)
            {
                ...
                newRow.Cells[colIndex].Text = GetFormatedString(s.FormatString, GetColumnFormat(GetColumnIndex(s.Column)), s.Value);
            }
        }
    ...
    private void GenerateGeneralSummaries(GridViewRowEventArgs e)
    {
        ...
        foreach (GridViewSummary s in mGeneralSummaries)
        {
            ...
            row.Cells[colIndex].Text = GetFormatedString(s.FormatString, GetColumnFormat(GetColumnIndex(s.Column)), s.Value);
        }
