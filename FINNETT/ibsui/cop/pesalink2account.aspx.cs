...
{
    public partial class pesalink2account : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if(!IsPostBack)
                {
                    LoadAccounts();
                    LoadBanks();
                    SourceAccountNumber.Text = SourceAccountName.SelectedValue;
                    trans_account.Text = SourceAccountName.SelectedValue;
                    fromdate.Text = DateTime.Now.ToShortDateString();
                    todate.Text = DateTime.Now.ToShortDateString();
                    LoadData(2, SourceAccountNumber.Text);
        ...
        protected void LoadData(int Code, string SearchParameter)
        {
            ...
            RestRequest request = new RestRequest("get/cop/pesalink/account", Method.POST);
        ...
        protected void add_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    RestClient client = new RestClient(SuiteDreamLiner.APIUrul());
                    RestRequest request = new RestRequest("cop/pesalink/account", Method.POST);
                    request.AddHeader("Accept", "application/json");
                    request.AddHeader("Authorization", Secure360.GetAuthorization());
                    request.AddHeader("Signature", Secure360.GetSignature());
                    var bodyParameters = new
                    {
                        SourceAccountNumber = SourceAccountNumber.Text,
                        Amount = amount.Text,
                        SourceTransactionCurrency = currency.SelectedValue,
                        SourceNarration = SourceNarration.Text,
                        ReferenceNumber = ReferenceNumber.Text,
                        DestinationsAccountNumber = DestinationsAccountNumber.Text,
                        DestinationsTransactionCurrency = currency.SelectedValue,
                        DestinationsNarration = DestinationsNarration.Text,
                        BankCode = bankID.SelectedValue
        ...
        protected void SourceSourceAccountNumber_SelectedIndexChanged(object sender, EventArgs e)
        {
            SourceAccountNumber.Text = DestinationsAccountNumber.Text;
        }
        ...
        protected void LoadAccounts()
        {
            ...
            SourceAccountName.DataSource = dt;
            SourceAccountName.DataValueField = "AccountNumber";
            SourceAccountName.DataTextField = "AccountName";
            SourceAccountName.DataBind();

            trans_accounts.DataSource = dt;
            trans_accounts.DataValueField = "AccountNumber";
            trans_accounts.DataTextField = "AccountName";
            trans_accounts.DataBind();
        ...
        protected void LoadBanks()
        {
            ...
            RestClient client = new RestClient(SuiteDreamLiner.APIUrul());
            RestRequest request = new RestRequest("get/all/banks", Method.POST);
            request.AddHeader("Accept", "application/json");
            request.AddHeader("Authorization", Secure360.GetAuthorization());
            request.AddHeader("Signature", Secure360.GetSignature());
            var bodyParameters = new
            ...
            bankID.DataValueField = "BankCode";
            bankID.DataTextField = "BankName";
            bankID.DataBind();
        ...
        protected void SourceAccountName_SelectedIndexChanged(object sender, EventArgs e)
        {
            SourceAccountNumber.Text = SourceAccountName.SelectedValue;
            trans_account.Text = SourceAccountName.SelectedValue;
            trans_accounts.SelectedValue = SourceAccountName.SelectedValue;
            LoadData(2, SourceAccountNumber.Text);
        }

        protected void getTransactions_Click(object sender, EventArgs e)
        {
            LoadData(3, SourceAccountNumber.Text);
        }

        protected void trans_accounts_SelectedIndexChanged(object sender, EventArgs e)
        {
            trans_account.Text = trans_accounts.SelectedValue;
        }
    }
}