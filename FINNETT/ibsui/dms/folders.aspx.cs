...
namespace FINNETT.ibsui.dms
{
    public partial class folders : System.Web.UI.Page
    {
        ...
        protected void update_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    RestRequest request = new RestRequest("update/Folder", Method.POST);
        ...
        protected void LoadData(int Code, string SearchParameter)
        {
            ...
            RestClient client = new RestClient(SuiteDreamLiner.APIUrul());
            RestRequest request = new RestRequest("get/folder", Method.POST);
            request.AddHeader("Accept", "application/json");
            request.AddHeader("Authorization", Secure360.GetAuthorization());
            request.AddHeader("Signature", Secure360.GetSignature());
            var bodyParameters = new
        ...
        protected void add_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    string hasError = string.Empty;
                    RestClient client = new RestClient(SuiteDreamLiner.APIUrul());
                    RestRequest request = new RestRequest("create/Folder", Method.POST);
                    request.AddHeader("Accept", "application/json");
                    request.AddHeader("Authorization", Secure360.GetAuthorization());
                    request.AddHeader("Signature", Secure360.GetSignature());
                    var bodyParameters = new
        ...
        protected void PreviewData(string pk)
        {
            ...
            {
                FolderName.Text = receivedData2["FolderName"].ToString();
                FolderPath.Text = receivedData2["FolderPath"].ToString();
                SourceID.Text = receivedData2["SourceID"].ToString();
                Size.Text = receivedData2["Size"].ToString();
                NumberOfItems.Text = receivedData2["NumberOfItems"].ToString();
                DateModified.Text = receivedData2["DateModified"].ToString();
                DateCreated.Text = receivedData2["DateCreated"].ToString();
